...
定義（id / ラベル / 解像度）は `src/main.ts` の `RATIOS` 配列を参照。

| 項目 | 内容 |
|------|------|
| 短辺 | 全画角1080pxで固定（被写体の見かけサイズを画角間で不変にするため） |
| 長辺 | 比率に応じて1080〜1920px |
| 背景の扱い | 1920x1920の原本をセンタークロップ（cover）。全画角で等倍以下のため拡大劣化なし |
| プレビュー高さ上限 | 60vh（縦長でも画面内に収まるようCSSで幅を決める） |
| 出力形式 | PNG（画角によらず `canvas.toBlob()`） |
