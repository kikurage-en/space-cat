...
4. 出力画角を選択（正方形 / 横長 / 縦長）
5. Canvas上で選択背景 + 被写体を合成
6. PNG出力（Web Share API / ダウンロード / クリップボード）
...
### 画角管理
画角の定義（id / ラベル / 出力解像度）は `src/main.ts` の `RATIOS` 配列が SSOT。
ボタンDOMと比率アイコンの寸法はこの配列から動的生成されるため、画角の追加・削除は配列の編集だけで完結する。

出力解像度は**短辺1080pxで固定**しており、これが2つの性質を担保する。
- 被写体は短辺を基準にスケールするため、画角を切り替えても見かけの大きさが変わらない
  （両軸に依存する contain fit だと、横長の被写体で切替時に大きさが跳ねる）
- 背景原本（1920x1920）がどの画角でも等倍以下に収まるため、拡大による劣化が起きない
  → 画角ごとの背景画像を用意する必要がなく、既存のセンタークロップで足りる

プレビューの表示比は `applyRatio()` がCSS変数 `--canvas-ar` として `.canvas-wrapper` に渡す。

### 被写体の状態管理
正規化座標(0-1)で管理: `subjectX`, `subjectY`, `subjectScale`, `subjectRotation`, `subjectFlipped`
正規化しているため、画角を切り替えても位置・サイズ・回転はそのまま維持される。
